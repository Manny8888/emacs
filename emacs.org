...
#+BEGIN_SRC emacs-lisp
  (use-package exwm
    ...
    ;; Ignore M-# in other applications (looking at you, Firefox):
    (push ?\M-0 exwm-input-prefix-keys)
    ...
    (defun my/mute ()
      "Mute"
      (interactive)
      (start-process "" nil "pa-adjust" "mute"))
    (defun my/volume-up ()
      "Volume up"
      (interactive)
      (start-process "" nil "pa-adjust" "plus"))
    (defun my/volume-down ()
      "Volume down"
      (interactive)
      (start-process "" nil "pa-adjust" "minus"))
    (defun my/mute-mic ()
    ...
    (defun my/brightness-up ()
      ...
      (interactive)
      (start-process "" nil "light" "-p" "-A" "10"))
    (defun my/brightness-down ()
      "Raise screen brightness"
      (interactive)
      (start-process "" nil "light" "-p" "-U" "10"))
    ...
    (exwm-input-set-key (kbd "<XF86MonBrightnessUp>") #'my/brightness-up)
    (exwm-input-set-key (kbd "<XF86MonBrightnessDown>") #'my/brightness-down)
    (exwm-input-set-key (kbd "s-l") #'my/lock-screen)
    ;; Enable EXWM
    (exwm-enable)
    ;; Start some daemons:
    (start-process "" nil "systemctl" "--user" "start" "davmail@alex.service")
    (start-process "" nil "systemctl" "--user" "start" "dropbox.service")
    (start-process "" nil "systemctl" "--user" "start" "redshift.service"))
#+END_SRC
...
To quickly access scratch press =my/map f12=:
#+BEGIN_SRC emacs-lisp
  (defun my/get-scratch ()
    "Switch to scratch buffer."
    (interactive)
    (switch-to-buffer "*scratch*"))
  (bind-key "<f12>" #'my/get-scratch 'my/map)
#+END_SRC
...
** Modeline
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package spaceline-config
       ...
       ;; Display date, time, and load average
...
This replaces =window-numbering= since =winum= can handle assigning numbers across multiple frames.
   ...
   #+END_SRC
** Window management
[[https://github.com/wasamasa/eyebrowse][Eyebrowse]] can manage window layouts (e.g. remembering what goes where). I change the default binding from =C-c C-w= to =s-w=:
#+BEGIN_SRC emacs-lisp
  (use-package eyebrowse
    :ensure t
    :config
    (setq eyebrowse-keymap-prefix "s-w")
    (eyebrowse-mode))
#+END_SRC
** Frame management
...
Typo mode replaces characters like --- with their unicode counterparts —. Because we're in the 21st century.
#+BEGIN_SRC emacs-lisp
  (use-package typo
    :ensure t
    :config
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package mu4e
       ...
       (setq mu4e-get-mail-command "mbsync -a"   ;; or fetchmail, or ...
             mu4e-update-interval 120)             ;; update every minute
